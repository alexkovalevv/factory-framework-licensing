...
add_action('onp_licensing_000_plugin_created', 'onp_licensing_000_plugin_created');
function onp_licensing_000_plugin_created( $plugin ) {
    $manager = new OnpLicensing000_Manager( $plugin );
...
class OnpLicensing000_Manager {
    ...
    public function activationHook() {
        ...
        if ( isset( $data['activate-trial'] ) && $data['activate-trial'] === true ) {
            ...
            factory_000_set_lazy_redirect($urlToRedirect);
    ...
    function addLicenseLinks($links) {
        $url = onp_licensing_000_get_manager_link( $this->plugin->pluginName );
        array_unshift($links, '<a href="' . $url . '" style="font-weight: bold;">'.__('License', 'onp_licensing_000'),'</a>');
        unset($links['edit']);
        return $links; 
    }
        ...
        if ( onp_build('premium', 'ultimate') ) {
            if ( $this->plugin->license && !$this->plugin->license->hasKey() ) {     
                if ( in_array( $this->plugin->license->default['Build'], array( 'premium', 'ultimate' ) ) ) {
                    $message = __('You use a premium version of the plugin. Please, verify your license key to unlock all its features. <a href="%1$s">Click here</a>.', 'onp_licensing_000');
                    $message = str_replace("%1\$s", onp_licensing_000_get_manager_link($this->plugin->pluginName), $message);
        ...
        if ( onp_build('free') ) {
            if ( $this->plugin->license && !$this->plugin->license->hasKey() ) { 
                ...
                if ( !isset( $current->response[ $this->plugin->relativePath ] ) ) {
                    ...
                    $message = __('Need more features? Look at a <a target="_blank" href="%1$s">premium version</a> of the plugin.', 'onp_licensing_000');
                    $message = str_replace("%1\$s", onp_licensing_000_get_purchase_url( $this->plugin ), $message);
                    return array($message);  
                }
            }
        }
    ...
    function addNotices( $notices ) {       
        ...
        if ( !factory_000_is_administrator() ) return $notices;
        ...
        if ( ( $time + 60*60*24 <= time() && ( !$this->plugin->updates || !$this->plugin->updates->needChangeAssembly() )) || $forceBindingMessage ) {        
            ...
            if ( ( $keyBound && $this->plugin->license && $this->plugin->license->key && $this->needToProtect() ) || $forceBindingMessage ) {
                ...
                $notices[] = array(
                    ...
                    'icon'      => 'fa fa-frown-o',  
                    'header'    => __('Your license key is not protected!', 'onp_licensing_000'),
                    'message'   => sprintf(__('Bind your license key (for %s) to your email address in order to avoid theft (it will take just a couple of seconds).', 'onp_licensing_000'), $this->plugin->options['title']),   
                    'plugin'    => $this->plugin->pluginName,
                    ...
                    'buttons'   => array(
                        array(
                            ...
                            'title'     => '<i class="fa fa-key"></i> ' . sprintf( __('Protect my key: %s', 'onp_licensing_000' ), '<i>' . $this->plugin->license->key . '</i>' ),
                        ...
                        array(
                            'title'     => __('Hide this message', 'onp_licensing_000'),
        ...
        if ( $exipred != 0 || $forceTrialNotices ) {
                    ...
                    if ( $this->type == 'trial' || $forceTrialNotices ) {
                        ...
                        if ( $remained <= 1  ) {
                            ...
                            $notices[] = array(
                                ...
                                'class'     => 'call-to-action',
                                'icon'      => 'fa fa-clock-o',   
                                'header'    => sprintf(__('The trial key for the %s will expire during the day!', 'onp_licensing_000'), $this->plugin->pluginTitle),
                                'message'   => __('Don\'t forget to purchase the premium key or delete the trial key to use the free version of the plugin.', 'onp_licensing_000'),   
                                'plugin'    => $this->plugin->pluginName,
                                
                                // buttons and links
                                'buttons'   => array(
                                    array(
                                        'title'     => '<i class="fa fa-arrow-circle-o-up"></i> '.__('Buy a premium key now!', 'onp_licensing_000'),
                                        'class'     => 'btn btn-primary',
                                        'action'    => onp_licensing_000_get_purchase_url( $this->plugin, 'trial-remained-1' )
                                    ),
                                    array(
                                        'title'     => __('Hide this message', 'onp_licensing_000'),
                                        'class'     => 'btn btn-default',
                            ...
                            $notices[] = array(
                                ...
                                'class'     => 'call-to-action',
                                'icon'      => 'fa fa-clock-o',
                                'header'    => sprintf(__('The trial key for the %s will expire in %s days.', 'onp_licensing_000'),$this->plugin->pluginTitle, $remained),
                                'message'   => __('Please don\'t forget to purchase the premium key or delete the trial key to use the free version of the plugin.', 'onp_licensing_000'),   
                                'plugin'    => $this->plugin->pluginName,
                                ...
                                'buttons'   => array(
                                    array(
                                        'title'     => '<i class="fa fa-arrow-circle-o-up"></i> '.__('Buy a premium key now!', 'onp_licensing_000'),
                                        'class'     => 'btn btn-primary',
                                        'action'    => onp_licensing_000_get_purchase_url( $this->plugin, 'trial-remained-' . $remained )
                                    ),
                                    array(
                                        'title'     => __('Hide this message', 'onp_licensing_000'),
                                        'class'     => 'btn btn-default',
            ...
            if ( $this->isExpired() || $forceTrialNotices ) {
                ...
                $notices[] = array(
                    ...
                    'header'    => sprintf(__('The trial key for the %s has expired.', 'onp_licensing_000'),$this->plugin->pluginTitle),
                    'message'   => __('Please purchase another key or delete the current key to use the free version of the plugin.', 'onp_licensing_000'),   
                    'plugin'    => $this->plugin->pluginName,
                    ...
                    'buttons'   => array(
                        array(
                            'title'     => '<i class="fa fa-arrow-circle-o-up"></i> '.__('Buy a premium key now!', 'onp_licensing_000'),
                            'class'     => 'btn btn-primary',
                            'action'    => onp_licensing_000_get_purchase_url( $this->plugin, 'trial-expired' )
                        ),
                        array(
                            'title'     => __('Visit the license manager', 'onp_licensing_000'),
                            'class'     => 'btn btn-default',
                            'action'    => onp_licensing_000_get_manager_link($this->plugin->pluginName, 'index')
                        ),
                    )
                );
            }
            ...
            if ( $this->plugin->license && !$this->plugin->license->hasKey() ) {
                if ( in_array( $this->plugin->license->default['Build'], array( 'premium', 'ultimate' ) ) ) {
                    ...
                    $notices[] = array(
                        ...
                        // content and color
                        'class'     => 'factory-hero',
                        'header'    => __('Thank you ', 'onp_licensing_000'),
                        'message'   => sprintf( __(' for purchasing <a target="_blank" href="%s" class="highlighted">%s</a>.', 'onp_licensing_000' ), onp_licensing_000_get_purchase_url( $this->plugin ), $this->plugin->pluginTitle, 'onp_licensing_000').
                            sprintf( __('Please verify your license key you got to unlock all the plugin features. Click the button on the right. Feel free to <a target="_blank" href="%s">contact us</a> if you need help.', 'onp_licensing_000'), $contactUrl ),
                        'plugin'    => $this->plugin->pluginName,
                        
                        // buttons and links
                        'buttons'   => array(
                            array(
                                'title'     => __('verify my license key', 'onp_licensing_000'),
                                'action'    => onp_licensing_000_get_manager_link($this->plugin->pluginName, 'index')
                            )
                        )
                    );
 ...
 * @param type $action
 */
function onp_licensing_000_manager_link( $pluginName, $action = null, $echo = true ) {
 ...
 * @param type $action
 */
function onp_licensing_000_get_manager_link( $pluginName, $action = null ) {
 ...
 * 
 * @since 3.0.7
 * @param Factory000_Plugin $plugin
 * @return void
 */
function onp_licensing_000_purchase_url( $plugin ) {
    echo onp_licensing_000_get_purchase_url( $plugin );
}
 ...
 * @return string
 */
function onp_licensing_000_get_purchase_url( $plugin, $campaign = 'upgrade-to-premium', $content = null ) {
